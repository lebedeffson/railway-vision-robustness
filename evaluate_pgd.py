...
# Случайная начальная точка внутри epsilon-окрестности.
RANDOM_START = True

SEED = 2026

# Удаляется только outputs/attacks/pgd.
RESET_OUTPUT = True
...
def check_environment() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError(
            "CUDA недоступна. PGD на CPU не запускается."
        )

    print("=" * 76)
    print("WHITE-BOX PGD ДЛЯ YOLO11m")
    print("=" * 76)
...
def prepare_output_directory() -> None:
    if OUTPUT_DIR.exists():
        ...
        print(
            "\nУдаляется предыдущая папка PGD:\n"
            f"{OUTPUT_DIR}"
        )
...
def prepare_model_configuration(
    ...
    """
    Преобразует model.args из dict в объект конфигурации.

    YOLO loss ожидает параметры:
    model.args.box
    model.args.cls
    model.args.dfl
    """
...
class PGDValidator(DetectionValidator):
    """
    Стандартный DetectionValidator с white-box PGD.

    PGD выполняется после штатного preprocess Ultralytics:
    - resize;
    - letterbox;
    - перенос на GPU;
    ...
    def __init__(
        ...
        if self.current_epsilon_pixels > 0:
            ...
            # Модель создаётся до входа валидатора
            # в torch.inference_mode.
            ...
            # Веса модели фиксируются.
            # Градиент нужен только по изображению.
            for parameter in (
            ...
            if inference_parameters:
                raise RuntimeError(
                    ...
                    "созданы как inference-тензоры:\n"
                    + "\n".join(
                        inference_parameters[:10]
                    )
                )

            # Training mode необходим для YOLO loss.
            self.attack_model.train()

            # BatchNorm не должен менять статистики.
            for module in (
    ...
    def create_normal_attack_batch(
        ...
        """
        Клонирует тензоры валидационного batch,
        чтобы убрать ограничения inference tensors.
        """
        ...
        for key, value in batch.items():
            if isinstance(
                ...
                if torch.is_inference(
                    ...
                    raise RuntimeError(
                        "После clone тензор остался "
                        f"inference tensor: {key}"
                    )
    ...
    def calculate_loss(
        ...
        if self.attack_model is None:
            raise RuntimeError(
                "Атакующая модель не загружена."
            )

        attack_batch["img"] = image
    ...
    def calculate_pgd(
        ...
        with (
            ...
            if torch.is_inference(clean):
                raise RuntimeError(
                    "Чистое изображение осталось "
                    "inference-тензором."
                )

            # Случайный старт внутри L-infinity шара.
            if RANDOM_START:
            ...
            for step_index in range(
                ...
                if not torch.isfinite(
                    ...
                    raise RuntimeError(
                        "Градиент PGD содержит NaN или Inf."
                    )

                # Градиентный подъём:
                # увеличиваем ошибку модели.
                updated = (
                    adversarial
                    + self.current_alpha
                    * gradient.sign()
                ...
                # Проекция в L-infinity окрестность
                # исходного изображения.
                delta = torch.clamp(
                    updated - clean,
                    min=-self.current_epsilon,
                    max=self.current_epsilon,
                )

                # Ограничение допустимого диапазона пикселей.
                adversarial = torch.clamp(
                    clean + delta,
                    min=0.0,
                    max=1.0,
        ...
        return (
            clean_detached,
            adversarial_detached,
            float(initial_loss_value or 0.0),
            final_loss_value,
        )
    ...
    def preprocess(
        ...
        # Штатный preprocess Ultralytics.
        ...
        self.initial_loss_sum += initial_loss
        self.final_loss_sum += final_loss
...
def evaluate_epsilon(
    ...
    # Отдельный seed для воспроизводимого random start.
    current_seed = (
        SEED
        + epsilon_pixels
    )
    ...
    PGDValidator.epsilon_pixels = (
        epsilon_pixels
    )

    PGDValidator.last_quality = {}
    ...
    if epsilon_pixels > 0:
        ...
        print(
            f"Alpha = "
            f"{alpha_pixels:.4f}/255"
        )

    print("=" * 76)

    metrics = evaluation_model.val(
        validator=PGDValidator,
...
def print_summary(
    ...
    print("\n" + "=" * 112)
    print(f"PGD-{PGD_STEPS} ЗАВЕРШЁН")
    print("=" * 112)
    ...
    for result in results:
        ...
        print(
            ...
            f"{result['alpha_pixels']:>8.2f}/255"
    ...
    print(
        "\nОбщая таблица:\n"
        f"{OUTPUT_DIR / 'pgd_summary.csv'}"
    )

    print(
        "\nМетрики по классам:\n"
        f"{OUTPUT_DIR / 'pgd_all_classes.csv'}"
    )

    print(
        "\nПолный JSON:\n"
        f"{OUTPUT_DIR / 'pgd_results.json'}"
    )
