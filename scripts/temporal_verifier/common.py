...
CONFIG = PROJECT / "configs/temporal_verifier_v1.yaml"
...
def assert_locked() -> dict[str, Any]:
    assert_test_sealed()
    if not LOCK.is_file():
        raise RuntimeError("Temporal verifier protocol is not locked")
    payload = json.loads(LOCK.read_text(encoding="utf-8"))
    if payload["config_sha256"] != sha256(CONFIG):
        raise RuntimeError("Verifier config changed after protocol lock")
    for relative, expected in payload["implementation_sha256"].items():
        path = PROJECT / relative
        if not path.is_file() or sha256(path) != expected:
            raise RuntimeError(f"Verifier implementation changed: {relative}")
    if payload["test_status"] != "SEALED" or payload["test_access_count"] != 0:
        raise RuntimeError("Verifier test isolation failed")
    return payload
